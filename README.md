...
### 프론트엔드 및 인프라를 다루는 개발자입니다 🤗

React · TypeScript 기반의 **프론트엔드**를 중심에 두고, 서비스가 사용자에게 닿기까지 필요한
**배포 · 확장성 · 인증 · 보안** 영역을 함께 맡습니다.
주로 멀티테넌트의 CI/CD와 다중 도메인 배포를 직접 굴리고, 트래픽이 몰리는 상황을 견디도록 인프라를 설계하며,
인증 경계와 공개 엔드포인트까지 점검합니다.
<br>

## 💻 Approach
* 증상과 원인을 분리하고, 문제가 어느 층에 있든 **끝까지 따라 내려가서** 원인을 확인합니다.
...
* 문제를 해결한 뒤, **재발하지 않도록** 설계 단계까지 고민합니다.
<br>
